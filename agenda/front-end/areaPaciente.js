document.addEventListener("DOMContentLoaded", async () => {
  ...
  function drawAreaCanvas(canvasId, labels, points){
    ...
    for(let i=0;i<points.length;i++){
      const x = i*stepX;
      const y = H - (points[i]/max)*(H-20);
      if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);
    }
    // fill
    ctx.lineTo(W,H); ctx.lineTo(0,H); ctx.closePath();
    ctx.fillStyle = 'rgba(31,111,227,0.12)'; ctx.fill();
    // stroke
    ctx.beginPath();
    for(let i=0;i<points.length;i++){
      const x = i*stepX;
    ...
    ctx.strokeStyle = '#1f6fe3'; ctx.lineWidth = 2; ctx.stroke();
  }
