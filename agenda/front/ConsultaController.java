...
import com.trabalho.agenda.model.*;
import com.trabalho.agenda.repository.*;
...
// função auxiliar: tenta mapear uma string (pt ou en) para DayOfWeek
    ...
    // 🔹 EXCLUIR CONSULTA (Usando seu método existente)
    @DeleteMapping("/{id}")
    public ResponseEntity<String> excluirConsulta(@PathVariable Long id) {
        try {
            // 1. Verifica se existe um prontuário ligado a essa consulta
            // Estamos usando o método que você já tem no repositório: findByConsultaId
            Prontuario prontuario = prontuarioRepository.findByConsultaId(id);

            // 2. Se existir, deleta o prontuário primeiro
            if (prontuario != null) {
                prontuarioRepository.delete(prontuario);
            }

            // 3. Agora que o prontuário sumiu, podemos deletar a consulta sem erro
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Erro ao remover consulta: " + e.getMessage());
        }
    }
}
